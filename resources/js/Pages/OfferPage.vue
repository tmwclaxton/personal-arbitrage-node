					...
					>
						<div ref="chatContainer" class="flex flex-col h-64 overflow-y-auto">
							<div
							  ...
							  :key="chatMessage.id"
							  :class="{'justify-end': chatMessage.user_nick === offer.ur_nick, 'justify-start': chatMessage.user_nick !== offer.ur_nick}"
							  class="flex flex-row p-2"
							>
								<div
								  :class="{
                'bg-blue-200 dark:bg-blue-600 text-right': chatMessage.user_nick === offer.ur_nick,
                'bg-gray-200 dark:bg-gray-700 text-left': chatMessage.user_nick !== offer.ur_nick
              }"
								  class="flex flex-col max-w-xs p-3 rounded-lg"
								>
									<p class="text-zinc-500 dark:text-zinc-200 font-bold">{{ chatMessage.user_nick }}</p>
									<p class="text-zinc-500 dark:text-zinc-200">{{ chatMessage.message }}</p>
								</div>
							</div>
						</div>
...
onMounted(() => {
	// Scroll to the bottom of the chat container when the component is mounted
