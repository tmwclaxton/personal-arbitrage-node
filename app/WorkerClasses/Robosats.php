...
//            dd($offer);
    ...
    public function updateTransactionStatus($offer) {
        ...
        if (isset($response['bad_request']) ) {
            ...
            if ($response['bad_request'] == RobosatsErrors::COLLABORATIVELY_CANCELLED_ERROR) {
            ...
            // robosats seems to return a bad_request response sometimes before trade hits a 14 completed status i.e. sending sats to the buyer, and we should just set expires_at to now rather than setting status to 99
            if ($offer->status < 14) {
...
//    http://robodexarjwtfryec556cjdz3dfa7u47saek6lkftnkgshvgg2kcumqd.onion
    ...
    public function togglePauseOffer($offer) {
        $url = $this->getHost() . '/mainnet/' . $offer->provider . '/api/order/?order_id=' . $offer->robosatsId;
