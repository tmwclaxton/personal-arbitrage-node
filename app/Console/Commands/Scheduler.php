...
class Scheduler extends Command
{
    ...
    public function handle()
    {
        ...
        if ($admin_dashboard->scheduler && !$admin_dashboard->panic_button) {
            ...
            $slack_service = new \App\Services\SlackService();
            if ($current_time >= $start_time && $current_time <= $end_time) {
                // turn on the auto accept
                if (!$admin_dashboard->autoAccept) {
                    $slack_service->sendMessage('Scheduler is turning on auto accept');
                    $admin_dashboard->autoAccept = true;
                    $admin_dashboard->save();
                }
                // turn on the auto create
                if (!$admin_dashboard->autoCreate) {
                    $slack_service->sendMessage('Scheduler is turning on auto create');
                ...
                if ($admin_dashboard->autoAccept) {
                    $slack_service->sendMessage('Scheduler is turning off auto accept');
                ...
                if ($admin_dashboard->autoCreate) {
                    $slack_service->sendMessage('Scheduler is turning off auto create');
